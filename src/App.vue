...
<v-navigation-drawer fixed :clipped="$vuetify.breakpoint.lgAndUp" right app v-model="readBMail">
 <mail-editor id="mailViewr"></mail-editor>
</v-navigation-drawer>
            ...
            <v-list dense>
                <template v-for="item in items">
                    ...
                    <v-list-group v-else-if="item.children" v-model="item.model" :key="item.text" :prepend-icon="item.model ? item.icon : item['icon-alt']"
                        ...
                        <v-list-tile v-for="(child, i) in item.children" :key="i" @keyup="teste">
        ...
        <v-toolbar color="blue darken-3" dark app :clipped-left="$vuetify.breakpoint.lgAndUp" fixed>
            ...
            <v-text-field flat solo-inverted prepend-icon="search" label="Search" class="hidden-sm-and-down" v-model="filterText"></v-text-field>
            <v-spacer></v-spacer>
            <v-btn icon>
        ...
        <v-content>
            <v-list two-line>
                <template v-for="(item, index) in getEmails()">
                    ...
                    <v-list-tile avatar v-else :key="item.title" @click="readBMail=!readBMail">
                        <v-avatar>
        ...
        <v-btn fab bottom right color="pink" dark fixed @click.stop="newEmail">
            <v-icon>add</v-icon>
        </v-btn>
        ...
        <v-dialog v-model="composeEmail">
            <v-card>
                <v-card-title class="grey lighten-4 py-4 title"> Compose email
                </v-card-title>
                <v-container grid-list-sm>
                    <v-layout row wrap>
                        <mail-editor id="mailWriter"></mail-editor>
                    </v-layout>
                </v-container>
                <v-card-actions>
                    <v-btn flat color="primary" @click="close">Cancel</v-btn>
                    <v-btn flat @click="sendEmail">Send</v-btn>
                </v-card-actions>
            </v-card>
        </v-dialog>
    </v-app>
